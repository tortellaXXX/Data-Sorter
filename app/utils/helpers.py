...
def wait_for_dremio_ready(timeout=300, interval=5):
    """
    Ждём пока порт Dremio станет доступен.
    Для первого запуска API может отвечать 403 или 405 — это нормально.
    """
    print("Waiting for Dremio to start...")
    start = time.time()
    ...
    while time.time() - start < timeout:
        try:
            ...
            if r.status_code in (403, 405):  # сервер жив, API отвечает
                print("Dremio is up!")
                return
            else:
                print(f"Got status {r.status_code}, waiting...")
...
def create_first_admin_user():
    ...
    try:
        ...
        if r.status_code in (200, 201):
            ...
            print("User already exists or forbidden (normal after first setup)")
        else:
            r.raise_for_status()
