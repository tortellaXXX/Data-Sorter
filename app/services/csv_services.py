...
def read_csv(contents: bytes, encoding: str = "utf-8") -> pd.DataFrame:
    ...
    """
    return pd.read_csv(StringIO(contents.decode(encoding, errors="ignore")))
...
def generate_preview_html(df: pd.DataFrame, sort_column: str, rows: int = 20, ascending: bool = True) -> str:
    """
...
def create_html_error(message: str) -> HTMLResponse:
    """
    Возврат HTML ошибки (готовый Response)
    """
...
def save_temp_csv(contents: bytes) -> str:
    ...
    """
    return create_temp_csv(contents)
